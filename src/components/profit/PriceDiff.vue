        ...
        <div class="profit-table-section">
            ...
            >
                ...
                <template #changePercent="{ value }">
                    <span class="change-percent" :class="getChangeClass(value)">
                        {{ formatPercent(value) }}
                    </span>
                </template>
                ...
                <template #returnRate="{ value }">
                    <span class="return-rate" :class="getChangeClass(value)">
                        {{ formatPercent(value) }}
                    </span>
                </template>
                ...
                <template #priceDiff="{ value }">
                    <span class="price-diff" :class="getChangeClass(value)">
                        {{ formatCurrency(value) }}
                    </span>
                </template>
            </VantTable>
        ...
        setup() {
            ...
            const stockProfitData = computed(() => {
                ...
                return userStockListStore.userStockList.map(stock => {
                    ...
                    // 計算市值（當前價格 × 股數）
                    const marketValue =
                        (stock.currentPrice || stock.price || 0) * (stock.shares || 0);
                    ...
                    const changePercent =
                        stock.buyPrice > 0
                            ? ((stock.currentPrice || stock.price || 0) - stock.buyPrice) /
                              stock.buyPrice
                            : 0;
            ...
            const summary = computed(() => {
                ...
                const totalMarketValue = data.reduce(
                    (sum, item) => sum + item.principal + item.priceDiff,
                    0
                );
